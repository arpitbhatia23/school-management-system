...
const register = asyncHandler(async (req, res) => {
    ...
    if (role === "student") {
        ...
        const { father_name, mother_name, father_occupation, phone, email } = parents;

        if ([father_name, mother_name, father_occupation,phone, email].some((field) => field?.trim()==="")) {
            throw new apiError(400, "All parent fields are required");
        }
        // Destructure and validate additional profile details
        const { className, DOB, religion, blood_group, nationality, address, category,admission_Date } = profile;
        if ([className, DOB, religion, blood_group, nationality, address, category,admission_Date].some((field) => field?.trim()==="")) {
