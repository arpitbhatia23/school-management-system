...
const StAttendance = () => {
  ...
  const fetchattendance = async (data) => {
    console.log(data)
    const res = await getMonthlyAttendance(data||{
    ...
    if (res.data.success) {
      ...
      toast({
        title:"failed",description:res?.data?.message,  variant: 'destructive',
      })
    }
  };
        ...
        <Form {...from}>
          <form onSubmit={from.handleSubmit(fetchattendance)}>
            ...
            render={({field})=>(
              <FormItem>
                <FormControl>
                  <Input placeholder="enter end date" type="date" {...field}/>
                </FormControl>
                <FormMessage/>
              </FormItem>
            )}/>
